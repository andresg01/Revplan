...
flowchart LR
  subgraph Mobile[App Móvil (React Native/Expo)]
    M1[Onboarding<br/>Preguntas rápidas]
    M2[Dashboard & Presupuesto]
    M3[Conectar Revolut]
  end
  subgraph Backend[Backend (Spring Boot / Java 21)]
    B1[OpenBanking<br/>(Tink/GoCardless o Revolut Business)]
    B2[Survey & Profile]
    B3[Plan Engine<br/>Reglas + Scoring]
    B4[LLM Adapter<br/>Responses API (GPT)]
    B5[Sync Service<br/>Cron/Webhooks]
    B6[Events<br/>SSE/WebSocket]
  end
  subgraph Data[PostgreSQL]
    D1[(Usuarios/Consentimientos)]
    D2[(Cuentas/Transacciones)]
    D3[(Perfiles/Respuestas)]
    D4[(PlanVersion/KPIs)]
  end
  M1 -->|/onboarding| B2
...
sequenceDiagram
  ...
  API->>OB: Create consent session
  OB-->>U: Login + Autorización
...
3. **Móvil** (Expo):
   ...
   npm start
   ```
4. Configura el **deep link** `myrevplan://ob/callback` y **redirect URI** correspondientes.
...
REDIRECT_URI=myrevplan://ob/callback
